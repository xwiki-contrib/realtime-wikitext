define(function () {
    ...
    var ELEMS = [ 'error', 'disconnected', 'debug', 'merge', 'parse', 'unavailable' ];
    ...
    for (var i = 0; i < ELEMS.length; i++) {
        elems[ELEMS[i]] = pageElem.getElementsByClassName('rtwiki-' + ELEMS[i])[0];
    }
    ...
    var show = module.exports.show = function (type, content, internalData) {
        ...
        new ModalPopup({ then: function (elem) {
            for (var name in elems) {
                ...
                var con = elem.getElementsByClassName('rtwiki-content')[0];
                if (con) { con.value = content; }
                var data = elem.getElementsByClassName('rtwiki-debug')[0];
